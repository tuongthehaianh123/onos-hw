...
type E2Cell struct {
	...
	HwReports      map[string]uint32 `protobuf:"bytes,7,rep,name=kpi_reports,json=kpiReports,proto3" json:"kpi_reports,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"varint,2,opt,name=value,proto3"`
...
func (m *E2Cell) GetHwReports() map[string]uint32 {
	if m != nil {
		return m.HwReports
	}
	return nil
...
func (m *E2Cell) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if len(m.HwReports) > 0 {
		for k := range m.HwReports {
			v := m.HwReports[k]
			baseI := i
...
func (m *E2Cell) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 7:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field HwReports", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.HwReports == nil {
				m.HwReports = make(map[string]uint32)
			}
			var mapkey string
			var mapvalue uint32
			for iNdEx < postIndex {
